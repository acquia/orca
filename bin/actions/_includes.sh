...
if [[ "$ORCA_JOB" ]]; then
  ...
  if [[ "$GITHUB_ACTIONS" ]]; then assert "$ORCA_SUT_BRANCH" "Missing required ORCA_SUT_BRANCH environment variable.\nHint: ORCA_SUT_BRANCH=8.x-1.x"; fi
fi
if [[ ! "$GITHUB_ACTIONS" && "$ORCA_JOB" = "STATIC_CODE_ANALYSIS" ]]; then
...
export ORCA_SUT_DIR=${ORCA_SUT_DIR:=${GITHUB_WORKSPACE}}
...
export PATH="$GITHUB_WORKSPACE/vendor/bin:$PATH"
